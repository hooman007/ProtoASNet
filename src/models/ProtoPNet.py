...
class PPNet(nn.Module):
    def __init__(
        ...
        first_add_on_layer_in_channels = self.get_cnn_backbone_out_channels(self.features)
    ...
    def conv_features(self, x):
        ...
        """
        x = self.features(x)  # [N, 3, 224, 224] --> [N, 1024, 7, 7]
    ...
    def get_cnn_backbone_out_channels(features):
        ...
        if "RESNET2P1D" in features_name:
            ...
            return [i for i in features.modules() if isinstance(i, nn.Conv2d)][-1].out_channels
        elif features_name.startswith("DENSE"):
            return [i for i in features.modules() if isinstance(i, nn.BatchNorm2d)][-1].num_features
        else:
